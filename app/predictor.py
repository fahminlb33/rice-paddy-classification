...
class PredictorModel:
    ...
    def make_gradcam_heatmap(self, img_array, last_conv_layer_name: str, pred_index=None):
        ...
        with tf.GradientTape() as tape:
            ...
            heatmap = tf.matmul(
                last_conv_layer_logits[0], pooled_grads[..., tf.newaxis])
    ...
    def predict(self, image_path: str, output_path: str, alpha: float = 0.4) -> Tuple[str, str]:
        ...
        # load image
        image_tensor = tf.keras.preprocessing.image.load_img(
            image_path, target_size=self.IMG_SIZE)
        image_tensor = tf.keras.preprocessing.image.img_to_array(image_tensor)
        image_tensor = tf.keras.applications.mobilenet.preprocess_input(
            image_tensor)
        ...
        superimposed_img: Image = tf.keras.preprocessing.image.array_to_img(
            superimposed_img)
